...
class Member:
    ...
    def create_member(input_data):
        ...
        DB.execute_input(sql, (input_data['fname'],input_data['lname'], input_data['account'], input_data['password'], input_data['identity']))
    ...
    @staticmethod
    def get_role(userid):
        sql = 'SELECT identity, lname FROM member WHERE mid = %s'
...
class Order_List:
    ...
    def get_order():
        sql = '''
            SELECT o.oid, m., o.price, o.ordertime
            FROM order_list o
...
class Analysis:
    ...
    @staticmethod
    def member_sale():
        sql = 'SELECT SUM(price), member.mid, member.lname FROM order_list, member WHERE order_list.mid = member.mid AND member.identity = %s GROUP BY member.mid, member.lname ORDER BY SUM(price) DESC'
        return DB.fetchall(sql, ('user',))

    @staticmethod
    def member_sale_count():
        sql = 'SELECT COUNT(*), member.mid, member.lname FROM order_list, member WHERE order_list.mid = member.mid AND member.identity = %s GROUP BY member.mid, member.lname ORDER BY COUNT(*) DESC'
        return DB.fetchall(sql, ('user',))
